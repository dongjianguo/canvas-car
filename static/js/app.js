...
var gameMonitor = {
  ...
  getScore : function(){
    ...
    var user = 1;
    if(score==0){
      $('#scorecontent').html('真遗憾，您竟然<span class="lighttext">一桶</span>油也没有抢到！');
    ...
    $('#scorecontent').html('您在<span id="stime" class="lighttext">2378</span>秒内准确抢了 <span id="sscore" class="lighttext">88</span>桶油<br>超过了<span id="suser" class="lighttext">88%</span>的用户!');
    $('#stime').text(time);
    $('#sscore').text(score);
    $('#suser').text(user+'%');
    $('.btn1').text('请小伙伴来抢油').removeClass('playagain').addClass('share');
  },
  isMobile : function(){
